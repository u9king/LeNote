...
##### 3.3.1 曝光补偿 Exposure Compensation

> 概念：在后期处理中对整个画面的亮度进行的调节，于相机的光圈或快门调节，用于控制进入镜头的光量

##### 3.3.2 MinEV100 /MaxEV100

MinEV100和MaxEV100是虚幻引擎中自动曝光Auto Exposure以及光照强度单位。

EV100 是指在 ISO100 时的曝光值Exposure Value。在虚幻引擎中，它被用作衡量场景亮度的物理标准单位。

MinEV100 与 MaxEV100 的定义了相机自动适配亮度的Clamp范围：

Min EV100自动曝光最小值： 这是自动曝光能够适配的最暗环境。它实际上决定了相机的最大亮度上限。如果这个值设得很高，相机在暗处就无法进一步开大光圈，画面就会显得很黑。

Max EV100自动曝光最大值： 这是自动曝光能够适配的最亮环境。它决定了相机的最小亮度下限。在极亮的环境下（如直视太阳），相机会根据这个值来收缩曝光。
