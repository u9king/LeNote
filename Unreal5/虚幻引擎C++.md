...
| 序号 | 说明符              | 编辑器原型面板表现 | 编辑器实例面板表现 | 蓝图逻辑连线表现       |
| :--: | ------------------- | ------------------ | ------------------ | ---------------------- |
|  1   | EditAnywhere        | 可见并且可编辑     | 可见并且可编辑     | 默认无法在连线中使用   |
|  2   | VisibleAnywhere     | 可见但不可编辑     | 可见但不可编辑     | 默认无法在连线中使用   |
|  3   | BlueprintReadWrite  | 无直接显示影响     | 无直接显示影响     | 可以获取值也可以设置值 |
|  4   | BlueprintReadOnly   | 无直接显示影响     | 无直接显示影响     | 只能获取值不能设置值   |
|  5   | EditDefaultsOnly    | 可见并且可编辑     | 隐藏且不可见       | 默认无法在连线中使用   |
|  6   | BlueprintAssignable | 不适用此面板       | 不适用此面板       | 仅用于委托绑定事件     |
|  7   | VisibleDefaultsOnly | 可见但不可编辑     | 隐藏且不可见       | 默认无法在连线中使用   |
|  8   | EditInstanceOnly    | 隐藏且不可见       | 可见并且可编辑     | 默认无法在连线中使用   |
|  9   | VisibleInstanceOnly | 隐藏且不可见       | 可见但不可编辑     | 默认无法在连线中使用   |
