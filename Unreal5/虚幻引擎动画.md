...
##### 5.Virtual Bones 虚拟骨骼

可以模仿IK骨骼

虚拟骨骼不是真正建出来的骨骼。它是为了方便连线、对齐或IK计算，而在现有的两个实体骨骼之间建立的一种连接。

##### 6.强制根锁定 Force Root Lock

控制动画原地播放In-Place

- 提取并冻结位移
- 角色原地播放
- **保留根运动数据**：最关键的是，它并没有删掉位移数据
...
> 作用：作为一个挂载点，用来把武器/特效/UI绑定到这个位置

##### 5.1 骨骼插槽 Skeleton Socket
...
- 它存在于具体的网格体（Mesh）实例层级，而不是共享的骨骼层级。
- 特点：独占性与特异性。它只对当前的网格体资产生效，不影响其他共享同一套骨骼的资产。


| 特性             | 骨骼插槽 Skeleton Socket         | 网格体插槽 Mesh Socket               |
| ---------------- | -------------------------------- | ------------------------------------ |
| 存在于谁体内     | 存在于Skeleton资产中             | 存在于 Skeletal / Static Mesh 资产中 |
| 静态网格体可用吗 | 不可用（静态网格体没有骨骼）     | 可用                                 |
| 复用性           | 极高。所有使用该骨骼的角色都能用 | 无复用。只针对当前这一个模型文件生效 |
| 位置微调         | 会影响所有使用该骨骼的角色       | 只会微调当前这一个模型的挂载位置     |



参考场景：体型差异极大的角色（微调挂点）

- 例子：游戏里，士兵和大胖子精英怪共享同一套骨骼。做了一个背枪的动画，在骨骼上加了 Back_Socket。
- 问题：因为大胖子模型太厚了，用通用的骨骼插槽会导致枪直接陷进肉里。
- 解决办法：你不需要去改动骨骼，只需要打开大胖子的Skeletal Mesh编辑器，找到继承过来的 Back_Socket，直接在这里修改它的相对位置和旋转（偏移量）。这个只属于大胖子的修改，就是网格体插槽的意义所在，它不会影响普通士兵的背枪位置。
...
#### 40.1 高级运动系统 Advanced Locomotion System

> 全称：Advanced Locomotion System V4
>
> 简称：ALS
...
- Audio
  ...
  - UI
- Blueprints
  - AnimModifiers：动画修改器，转向动画
  - AnimNotifys：动画通知
  - CameraSystem：摄像机系统
  - CharacterLogic：游戏逻辑
  - GameModes：游戏模式
  - Interfaces：接口，连接玩家动画和控制器
  - Libraries：蓝图库
  - Misc：杂项
  - UI
- CharacterAssets
...
项目：https://www.fab.com/listings/ef9651a4-fb55-4866-a2d9-1b38b028f9c7
...
GAMES104:https://www.bilibili.com/video/BV1jr4y1t7WR

CSDN:https://blog.csdn.net/ttm2d/article/details/119698520

高级运动系统(ALSV4)扩展教程：https://www.bilibili.com/video/BV17Hd2Y6EeH

付费：https://www.bilibili.com/cheese/play/ss20136
