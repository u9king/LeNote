...
漫反射纹理(Diffuse/Base Color):负责定义模型的基础颜色，是最核心的基础纹理，不包含光影信息，仅提供颜色基底。



功能:定义物体固有基准色彩，Alpha通道负责控制材质透明度，是色彩与通透度的双重控制节点。
调节方式:颜色部分通过选色器或RGB数値调整;Alpha通道通过调节对应数値(0-1)控制，0为完全透明，1为完全不透明，中间値为半透明效果。
