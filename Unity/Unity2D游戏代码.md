...
public class MainMenu : MonoBehaviour
{
    ...
    public Slider slider;   //进度条数值
    public Text progressText;  //进度条数字

    public void LoadLevel(int sceneInedx)
    {
        StartCoroutine(AsyncLoadLevel(sceneInedx));
    }

    IEnumerator AsyncLoadLevel(int sceneInedx)
    {
        AsyncOperation operation = SceneManager.LoadSceneAsync(sceneInedx);
        loadingPanel.SetActive(true);
        while (!operation.isDone) 
        {
            float progress = operation.progress / 0.9f;		//progress的取值是[0,0.9]
            slider.value = progress;	//进度条赋值
            progressText.text = Mathf.FloorToInt(progress * 100f) + "%";  //编写百分号
            yield return null;
        }
    }
...
### 五.脚本设计模式（Design Mode）

#### 1.组件化设计

**组件化设计：** 将不同功能划分为独立的组件，每个组件负责一个特定的功能。这有助于保持代码的清晰度，使得每个组件都可以被单独测试和修改。
...
**状态管理机（State Management）：** 使用状态机来管理对象的状态。这对于处理对象的不同行为和状态切换非常有用，尤其是在处理角色行为时。
...
**观察者模式（Observer Pattern）：** 用于实现一对多的依赖关系，当一个对象的状态发生变化时，所有依赖它的对象都会得到通知。
...
**命令模式（Command Pattern）：** 将一个请求封装为一个对象，从而使你可以参数化客户端对象，排队请求，或者对请求进行记录和撤销。
